...
<template>
  <div class="border-b pb-10 pt-20 px-3">
    <h1 class="uppercase text-4xl md:text-6xl lg:text-8xl xl:text-9xl w-1/2 pt-4">
      Tout mes projets
    </h1>
    ...
    <div class="flex flex-col gap-10 mt-10">
      <div v-for="projet in data" class="w-full border-t flex flex-wrap justify-between pt-8">
        <NuxtLink :to="'/projet/' + projet.uid" class="flex flex-col w-1/2 md:justify-end gap-4 cursor-none">
          <div class="flex gap-2 text-xs md:text-base lg:text-lg">
            <p v-for="tag in projet.data.tags" class="px-2 py-[2px] uppercase flex items-center border rounded-full">
              {{ tag?.text }}
            </p>
          </div>
          <h2 class="text-2xl md:text-5xl lg:text-6xl xl:text-7xl uppercase w-fit">
            {{ projet.data.titre_du_projet[0]?.text }}
          </h2>
        </NuxtLink>
        <NuxtLink :to="'/projet/' + projet.uid"
          class="w-full md:h-auto md:w-2/5 h-full max-h-[28rem] xl:max-h-[40rem] flex cursor-none overflow-hidden">
          <img :src="projet.data.image_presentation.url" :alt="projet.data.image_presentation?.alt"
